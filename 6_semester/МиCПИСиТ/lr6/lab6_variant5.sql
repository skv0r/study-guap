-- лр-6, вариант 5
-- хранимые процедуры и функции (PostgreSQL)
--
-- 1) вставка студента с пополнением справочника «Группа», если пары (факультет, номер группы) ещё нет
-- 2) удаление студента; если группа опустела — строка «Группа» удаляется
-- 3) каскадное удаление группы: участия в выступлениях, студенты, затем группа
-- 4) скалярная функция — среднее число участий в «Выступление_студент» на одного студента вуза
-- 5) процедура заполняет временную таблицу статистикой по факультетам вуза «ГУАП»
-- 6) цикл по конференциям — число участий в выступлениях по каждой (временная таблица)
-- 7) табличная функция — студенты, выступавшие на указанной конференции
...
-- пример вызова (меняет данные; при необходимости раскомментировать)
