...
function sendTemperatureCommand() {
  $.ajax({
    ...
    success: function (r) {
      if (r.ok) alert("Температура установлена: " + r.value);
    },
  });
}
...
function sendLockCommand() {
  $.ajax({
    ...
    url: "/command/lock",
    dataType: "json",
    data: { locked: document.getElementById("lock_cmd").value },
    success: function (r) {
      if (r.ok) alert("Замок: " + (r.locked ? "закрыт" : "открыт"));
    },
  });
}
