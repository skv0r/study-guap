...
/** ЛР9: данные для графика (ось X — timeStamp, ось Y — температура). */
app.get("/chart/temperature-series", async (req, res) => {
    const limit = Number(req.query.limit ?? 40);
    const lim = Number.isNaN(limit) ? 40 : limit;
    const series = await logger.getTemperatureChartSeries(lim);
    res.json(series);
});

void (async function main() {
    app.listen(PORT, () => {
        console.log(`http://127.0.0.1:${PORT}/ — ЛР9 (график: /chart.html, API /chart/temperature-series)`);
    });
    await logger.init();
    if (!logger.isEnabled()) {
        console.warn("MongoDB недоступна — график будет пустым, главная страница откроется");
    }
})().catch((e) => {
    console.error(e);
    process.exit(1);
