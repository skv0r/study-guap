...
/** ЛР6: обмен данными между «датчиком» и исполнителем (пособие — класс Heater, auto_power). */
export class HeaterController extends Thing {
    ...
    autoPower(temperatureC: number): void {
        ...
        console.log(
            `[HeaterController.autoPower] ${this.id}: t=${temperatureC.toFixed(1)}°C, порог=${this.switchOnBelowC}°C -> ${this.power}`
        );
    }
