...
void (async function main() {
    app.listen(PORT, () => {
        console.log(`http://127.0.0.1:${PORT}/ — ЛР7 (журнал MongoDB: Temperature, Heater)`);
    });
    await logger.init();
    if (!logger.isEnabled()) {
        console.warn("MongoDB недоступна — задайте MONGODB_URI или запустите mongod (сайт всё равно работает)");
    }
})().catch((e) => {
    console.error(e);
    process.exit(1);
