...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            android.util.Log.d("MainActivity", "MapKit инициализирован с API ключом")
        } catch (e: Exception) {
            android.util.Log.e("MainActivity", "Ошибка инициализации MapKit: ${e.message}")
            e.printStackTrace()
        }
        ...
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                R.id.menu_driver -> {
                    ...
                    // Показываем FAB кнопки только для вкладки гонщиков
                    fabAdd.show()
                    fabRefresh.show()
                    true
                }
                R.id.menu_cards -> {
                    // Показываем фрагмент карты Москвы
                    supportFragmentManager.commit {
                        replace(R.id.fragmentContainer, FragmentMap.newInstance())
                    }
                    // Скрываем FAB кнопки для карты
                    fabAdd.hide()
                    fabRefresh.hide()
                    true
                }
                R.id.menu_account -> {
                    ...
                    // Скрываем FAB кнопки для аккаунта
                    fabAdd.hide()
                    fabRefresh.hide()
                    true
                }
    ...
    override fun onStart() {
        ...
        try {
            ...
            android.util.Log.d("MainActivity", "MapKit запущен")
        } catch (e: Exception) {
            android.util.Log.e("MainActivity", "Ошибка запуска MapKit: ${e.message}")
        }
    }
    ...
    override fun onStop() {
        try {
            ...
            android.util.Log.d("MainActivity", "MapKit остановлен")
        } catch (e: Exception) {
            android.util.Log.e("MainActivity", "Ошибка остановки MapKit: ${e.message}")
        }
        super.onStop()
    }
